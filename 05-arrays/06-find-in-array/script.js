...
(function() {
    ...
    document.getElementById("run").onclick =  function() {
        /*var x = people.map(function(item, index){
            if(((people[index].firstname) == "Jean") && ((people[index].lastname) == "Dupont")){
            console.log(people[index].email);
            console.log(index);
            document.getElementById("target").innerHTML = (people[index].email);
            //return index;//people[index].email;

            }
        });*/
        
        //(numbers.find(x => x % 2 == 0));
        var ndx = (people.indexOf(people.find(pers => ((pers.firstname =="Jean") && (pers.lastname == "Dupont")))));
        console.log(people[ndx].email);
        console.log(ndx);
